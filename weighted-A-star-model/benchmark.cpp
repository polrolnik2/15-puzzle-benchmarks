...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--heuristic-weight" && i + 1 < argc) { heuristic_weight = stof(argv[++i]); }
    ...
    int visited_nodes = 0;
    vector<State> path = PuzzleSolveWeightedAstar(start_state, goal_state, weights, heuristic_weight, &visited_nodes);
