...
extern "C" {
    // Run weighted A* on a given instance file; returns time in milliseconds.
    // Returns 1 if solution found, 0 otherwise. visited_nodes and steps are outputs.
    int weighted_astar_run_instance(
        ...
        if (!input_file || !out_time_ms || !out_steps || !out_visited || !weights || weights_len <= 0) {
            return -1;
        }
        ...
        std::vector<int> w(weights, weights + weights_len);
        auto t0 = std::chrono::steady_clock::now();
        int visited_nodes = 0;
        std::vector<State> path = PuzzleSolveWeightedAstar(start_state, goal_state, w, heuristic_weight, &visited_nodes);
