...
extern "C" {
    // Run A* on a given instance file; returns time in milliseconds.
    // Returns 1 if solution found, 0 otherwise. visited_nodes and steps are outputs.
    int astar_run_instance(
        ...
        int visited_nodes = 0;
        std::vector<State> path = PuzzleSolveAstar(start_state, goal_state, w, &visited_nodes);
